function Bode_plot_report_ready_RL()
    clc; clearvars; close all;

    %% File
    filepath = 'Bode Plot.txt';
    if ~isfile(filepath)
    ...
    stepBlocks = strsplit(filetext, 'Step Information: L=');
    ...
    for i = 2:length(stepBlocks)
        ...
        % First token is inductance label
        tokens = textscan(block, '%s', 1);
        L_val = strtrim(tokens{1}{1});
        ...
        status = classify_stability(pm, gm, 45, 6);

        % Store
        L_list{end+1,1}    = L_val;
    ...
    T = table( ...
        ...
        PM_list, ...
        Stat_list, ...
        'VariableNames', {'L','fgc_MHz','fpc_MHz','GM_dB','PM_deg','Status'});

    %% Build display table with readable missing entries
    Display = table;
    Display.L = T.L;
    Display.fgc_MHz = cell(height(T),1);
    Display.fpc_MHz = cell(height(T),1);
    Display.GM_dB   = cell(height(T),1);
    Display.PM_deg  = cell(height(T),1);
    Display.Status  = T.Status;

    for i = 1:height(T)
        if isnan(T.fgc_MHz(i))
            Display.fgc_MHz{i} = 'N/A';
        else
            Display.fgc_MHz{i} = sprintf('%.4f', T.fgc_MHz(i));
        end

        if isnan(T.fpc_MHz(i))
            Display.fpc_MHz{i} = 'Out of range';
        else
            Display.fpc_MHz{i} = sprintf('%.4f', T.fpc_MHz(i));
        end

        if isnan(T.GM_dB(i))
            Display.GM_dB{i} = 'Out of range';
        else
            Display.GM_dB{i} = sprintf('%.3f', T.GM_dB(i));
        end

        if isnan(T.PM_deg(i))
            Display.PM_deg{i} = 'Out of range';
        else
            Display.PM_deg{i} = sprintf('%.3f', T.PM_deg(i));
        end
    end

    %% Neat command-window table
    fprintf('\n%-8s %-14s %-14s %-14s %-14s %-18s\n', ...
        'L', 'f_gc(MHz)', 'f_pc(MHz)', 'GM(dB)', 'PM(deg)', 'Status');
    fprintf('%s\n', repmat('-', 1, 88));

    for i = 1:height(Display)
        fprintf('%-8s %-14s %-14s %-14s %-14s %-18s\n', ...
            Display.L{i}, Display.fgc_MHz{i}, Display.fpc_MHz{i}, ...
            Display.GM_dB{i}, Display.PM_deg{i}, Display.Status{i});
    end

    %% Colored table figure
    figTable = uifigure('Name','Extracted Loop Gain Parameters', ...
                        'Position',[150 150 980 420], ...
                        'Color','w');

    cellData = table2cell(Display);

    uit = uitable(figTable, ...
        'Data', cellData, ...
        'ColumnName', {'L','f_gc (MHz)','f_pc (MHz)','GM (dB)','PM (deg)','Status'}, ...
        'Position', [10 10 960 400], ...
        'FontSize', 11);

    % Apply row colors based on status
    for i = 1:height(Display)
        style = uistyle;
        switch Display.Status{i}
            case 'Stable'
    ...
    %% Optional save
    saveas(fig, 'LoopGain_BodePlot_RL.png');
    exportapp(figTable, 'LoopGain_Table_RL.png');
    writetable(T, 'LoopGain_Extracted_Parameters_RL.csv');
end

function status = classify_stability(pm, gm, PM_stable, GM_stable)
    % Both unavailable
